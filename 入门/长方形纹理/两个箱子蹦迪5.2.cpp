...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //第一个参数是位置信息，第二个参数告诉opengl我们要发送多少矩阵
//第三个参数询问我们是否希望对我们的矩阵进行置换，也就是交换行和列
...
//因此我们要先用GLM的自带的函数value_ptr来变换这些数据。
        ...
        trans = glm::mat4(1.0f);
        trans = glm::translate(trans, glm::vec3(-0.5f, 0.5f, 0.0f));
        float nowtime = sin(glfwGetTime());
        nowtime = fabs(nowtime);
        trans = glm::scale(trans, glm::vec3(nowtime, nowtime, nowtime));
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans));
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
